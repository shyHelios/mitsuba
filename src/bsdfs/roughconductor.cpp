...
public:
    ...
    Spectrum eval(const BSDFSamplingRecord &bRec, EMeasure measure) const
    {
        ...
        MicrofacetDistribution distr(m_type, m_alphaU->eval(bRec.its).average(), m_alphaV->eval(bRec.its).average(),
                                     m_sampleVisible);
    ...
    Spectrum sample(BSDFSamplingRecord &bRec, const Point2 &sample) const
    {
        ...
        Spectrum F = fresnelConductorExact(dot(bRec.wi, m), m_eta, m_k) * m_specularReflectance->eval(bRec.its);
