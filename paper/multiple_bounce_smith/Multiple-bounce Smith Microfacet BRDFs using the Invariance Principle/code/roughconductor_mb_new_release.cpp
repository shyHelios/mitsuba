/*
这是论文Multiple-bounce Smith Microfacet BRDFs using the Invariance Principle的代码实现
*/
#include <chrono>
#include <fstream>
#include <iomanip>
...
struct Vertex {
    ...
    Vertex(const Vector &_wi, const Vector &_wo, float _pdf, const Spectrum &_weight, const Spectrum &_specAcc,
           float _pdfAcc, float _inLamda, float _outLamda, float _invPdf, float _invPdfAcc)
    {
        wi        = _wi;
        wo        = _wo;
        pdf       = _pdf;
...
struct PathSample {
    ...
    void add(const Vector &_wi, const Vector &_wo, float _pdf, const Spectrum &_weight, const Spectrum &_specAcc,
             float _pdfAcc, float inLamda, float outLamda, float invPdf, float _invPdfAcc)
    {
...
public:
    ...
    Spectrum eval(const BSDFSamplingRecord &bRec, EMeasure measure) const override
    {
        ...
        auto result = m_bdpt ? evalBDPT_new(pRec, m_order, measure, alpha_x, alpha_y)
    ...
    void samplePath(const Vector &wi_, Sampler *sampler, int order, PathSample &path, bool back, float alpha_x,
                    float alpha_y) const
    {
        float pdf       = 1.0f;
    ...
    Spectrum evalPT_new(BSDFSamplingRecord &bRec, int order, EMeasure measure, float alpha_x, float alpha_y) const
    {
        ...
        // 加上单次散射的f*cos(theta)项
        result += computeD_F(w0, woN, alpha_x, alpha_y) * s.getSk();

        for (int i = 1; i < order; ++i)
        {
            sampleVNDF(bRec, alpha_x, alpha_y);
            // 中间bounce的顶点项和pdf约分之后得到F / G1，这里1/G1=lambda(wi)乘积通过invPDF计算，F乘积通过weight计算
    ...
    Spectrum sample(BSDFSamplingRecord &bRec, Float &pdf, const Point2 &sample) const override
    {
        ...
        std::vector<RayInfo> dirList;
        // 有m_order次bounce，对应m_order+1个方向
        dirList.resize(m_order + 1);
        // RayInfo方向对应光线传播路径-w0
        ...
        while (true)
        {
            ...
            // pdf乘上G1项是因为顶点项分子上有个绝对值lambda(wi) = lambda(wi) + 1 = 1 / G1
            // 为什么不在weight上直接乘上绝对值lambda(wi)
            ...
            // the ray starts the next bounce，采样可见法线和计算菲涅尔项要求入射方向远离交点
            bRec.wi = -bRec.wo;
        }
        // 计算这条路径的Sk
...
public:
    ...
    void resolve(const GPUProgram *program, const std::string &evalName, std::vector<int> &parameterIDs) const override
    {
    ...
    void generateCode(std::ostringstream &oss, const std::string &evalName,
                      const std::vector<std::string> &depNames) const override
    {
