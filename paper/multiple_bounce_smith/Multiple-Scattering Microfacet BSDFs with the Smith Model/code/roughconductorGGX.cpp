...
vec3 samplePhaseFunction_conductor(const vec3 &wi, const float alpha_x, const float alpha_y, const Spectrum &m_eta,
                                   const Spectrum &m_k, Spectrum &weight)
{
    ...
    {
        if (wi.z > 0)
            wm = vec3(0.0f, 0.0f, 1.0f);
        else
            wm = normalize(vec3(wi.x, wi.y, 0.0f));
    }
    else
...
Spectrum evalPhaseFunction_conductor(const RayInfo &ray, const vec3 &wo, const float alpha_x, const float alpha_y,
                                     const Spectrum &m_eta, const Spectrum &m_k)
{
    if (ray.w.z > 0.9999f)
    ...
    // value
    const Spectrum value = fresnelConductorExact(dot(-ray.w, wh), m_eta, m_k) * std::max(0.0f, dot(-ray.w, wh)) *
                           D_ggx(wh, alpha_x, alpha_y) / 4.0f / projectedArea / dot(-ray.w, wh);
    return value;
}
...
Spectrum eval_conductor(const vec3 &wi, const vec3 &wo, const float alpha_x, const float alpha_y, const Spectrum &m_eta,
                        const Spectrum &m_k, const int scatteringOrderMax)
{
    if (wi.z <= 0 || wo.z <= 0)
        return Spectrum(0.0f);
    ...
    while (current_scatteringOrder < scatteringOrderMax)
    {
        ...
        // next direction
        Spectrum weight;
        ray.updateDirection(samplePhaseFunction_conductor(-ray.w, alpha_x, alpha_y, m_eta, m_k, weight), alpha_x,
                            alpha_y);
        energy = energy * weight;
        ray.updateHeight(ray.h);

        // 初始化wi_MISweight的时候ray.w就是next direction中updateDirection采样得到的新方向
...
vec3 sample_conductor(const vec3 &wi, const float alpha_x, const float alpha_y, const Spectrum &m_eta,
                      const Spectrum &m_k, const int scatteringOrderMax, Spectrum &energy)
{
    ...
    while (true)
    {
        ...
        ray.updateDirection(samplePhaseFunction_conductor(-ray.w, alpha_x, alpha_y, m_eta, m_k, weight), alpha_x,
                            alpha_y);
        energy = energy * weight;
        // 更新完direction后需要计算G1值，因此在这里有看似重复的更新高度操作
        ray.updateHeight(ray.h);
...
public:
    RoughConductor(const Properties &props) : BSDF(props)
    {
        ...
        else
        {
            ...
            intK.fromContinuousSpectrum(
                    InterpolatedSpectrum(fResolver->resolve("data/ior/" + materialName + ".k.spd")));
        }
    ...
    Spectrum eval(const BSDFSamplingRecord &bRec, EMeasure measure) const
    {
        ...
        // 无论前向还是后向，eval_conductor得到f(wi, wo)*cos(theta_o)，注意，是cos(theta_o)！
    ...
    Float pdf(const BSDFSamplingRecord &bRec, EMeasure measure) const
    {
        ...
        return D_ggx(wh, alpha_x, alpha_y) / (1.0f + ray.Lambda) / (4.0f * Frame::cosTheta(bRec.wi)) +
...
public:
    RoughConductorShader(Renderer *renderer, const Texture *specularReflectance, const Texture *alphaU,
                         const Texture *alphaV, const Spectrum &eta, const Spectrum &k) :
